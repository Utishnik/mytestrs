...
// ---------------- MiMalloc (baseline) ----------------
#[divan::bench(name = "mimm/smt", sample_count = 15)]
fn mimm_smt() {
    mimm(true);
}

#[divan::bench(name = "mimm/no-smt", sample_count = 15)]
fn mimm_no_smt() {
    mimm(false);
}

#[divan::bench(name = "mimm_light/smt", sample_count = 15)]
fn mimm_light_smt() {
    mimm_light(true);
}

#[divan::bench(name = "mimm_light/no-smt", sample_count = 15)]
fn mimm_light_no_smt() {
    mimm_light(false);
}

// ---------------- Per-thread bumpalo ----------------
#[divan::bench(name = "bump_scope_m/smt", sample_count = 15)]
fn bump_scope_m_smt() {
    let (fb, _, _, _) = pgo();
    bump_scope_m(fb, true);
}

#[divan::bench(name = "bump_scope_m/no-smt", sample_count = 15)]
fn bump_scope_m_no_smt() {
    let (fb, _, _, _) = pgo();
    bump_scope_m(fb, false);
}

#[divan::bench(name = "bump_scope_m_light/smt", sample_count = 15)]
fn bump_scope_m_light_smt() {
    let (_, lb, _, _) = pgo();
    bump_scope_m_light(lb, true);
}

#[divan::bench(name = "bump_scope_m_light/no-smt", sample_count = 15)]
...
#[divan::bench(name = "bump_shared_m/smt", sample_count = 15)]
fn bump_shared_m_smt() {
    let (fb, _, _, _) = pgo();
    bump_shared_m(fb, true);
}

#[divan::bench(name = "bump_shared_m/no-smt", sample_count = 15)]
fn bump_shared_m_no_smt() {
    let (fb, _, _, _) = pgo();
    bump_shared_m(fb, false);
}

#[divan::bench(name = "bump_shared_m_light/smt", sample_count = 15)]
fn bump_shared_m_light_smt() {
    let (_, lb, _, _) = pgo();
    bump_shared_m_light(lb, true);
}

#[divan::bench(name = "bump_shared_m_light/no-smt", sample_count = 15)]
...
#[divan::bench(name = "arena_full/smt", sample_count = 15)]
fn arena_full_smt() {
    let (_, _, fa, _) = pgo();
    arena_full(fa, true);
}

#[divan::bench(name = "arena_full/no-smt", sample_count = 15)]
fn arena_full_no_smt() {
    let (_, _, fa, _) = pgo();
    arena_full(fa, false);
}

#[divan::bench(name = "arena_light/smt", sample_count = 15)]
fn arena_light_smt() {
    let (_, _, _, la) = pgo();
    arena_light(la, true);
}

#[divan::bench(name = "arena_light/no-smt", sample_count = 15)]
...
#[divan::bench(name = "arena_full/forward/smt", sample_count = 15)]
fn arena_full_forward_smt() {
    let (_, _, fa, _) = pgo();
    arena_full_dir(fa, true, BumpDir::Forward);
}

#[divan::bench(name = "arena_full/forward/no-smt", sample_count = 15)]
...
#[divan::bench(name = "arena_full/backward/smt", sample_count = 15)]
fn arena_full_backward_smt() {
    let (_, _, fa, _) = pgo();
    arena_full_dir(fa, true, BumpDir::Backward);
}

#[divan::bench(name = "arena_full/backward/no-smt", sample_count = 15)]
...
#[divan::bench(name = "arena_full/middleout/smt", sample_count = 15)]
fn arena_full_middleout_smt() {
    let (_, _, fa, _) = pgo();
    arena_full_dir(fa, true, BumpDir::MiddleOut);
}

#[divan::bench(name = "arena_full/middleout/no-smt", sample_count = 15)]
...
#[divan::bench(name = "arena_full/neighbors/smt", sample_count = 15)]
fn arena_full_neighbors_smt() {
    let (_, _, fa, _) = pgo();
    arena_full_neighbors(fa, true);
}

#[divan::bench(name = "arena_full/neighbors/no-smt", sample_count = 15)]
